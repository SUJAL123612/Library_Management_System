...
export async function GET() {
  try {
    ...
    const fines = result.rows.map((row: any) => {
      const { overdueDays, fine } = calculateFine(row.issue_date, row.return_date);
      return {
    ...
    return NextResponse.json({ message: "Database error fetching fines" }, { status: 500 });
  }
}

export async function DELETE() {
  try {
    await db.query("DELETE FROM issued_books WHERE return_date IS NOT NULL");
    return NextResponse.json({ message: "All fines deleted successfully." });
  } catch (error) {
    console.error("Fine delete error:", error);
    return NextResponse.json({ message: "Error deleting fines" }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    if (!issue_id) {
      return NextResponse.json({ message: "issue_id is required" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Error updating fine" }, { status: 500 });
  }
}